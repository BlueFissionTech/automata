...
Module execution that needs explicit authorization, trace lineage, normalized
errors, and honest cancellation or limit semantics should use the additive
[`AgentModuleLifecycle` contract](agent-module-lifecycle.md). Feature discovery
does not authorize a run, and synchronous execution does not imply in-flight
cancellation, streaming, resume, or exactly-once effects.
